  ...
  return (
    <div className="bg-white rounded-3xl p-5 card-shadow relative overflow-hidden grain space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-bold text-sage-600 bg-sage-50 px-2.5 py-1 rounded-full">
          Week {adaptiveGoal.weekNumber}
        </span>
      </div>

      <div className="flex items-baseline gap-2">
        <span className="text-2xl font-extrabold text-warm-800 tabular-nums">{adaptiveGoal.currentGoal.toLocaleString()}</span>
        <span className="text-xs text-warm-400 font-medium">steps/day</span>
        <span className="text-[10px] font-bold text-sage-600 ml-auto bg-sage-50 px-2 py-0.5 rounded-full">+{improvement}%</span>
      </div>

      <div className="flex gap-3 text-center">
        <div className="flex-1 bg-cream-100 rounded-2xl py-2.5">
          <p className="text-sm font-extrabold text-warm-600 tabular-nums">{adaptiveGoal.baseline.toLocaleString()}</p>
          <p className="text-[9px] text-warm-400 font-medium">Baseline</p>
        </div>
        <div className="flex-1 bg-sage-50 rounded-2xl py-2.5">
          <p className="text-sm font-extrabold text-sage-700 tabular-nums">{adaptiveGoal.currentGoal.toLocaleString()}</p>
          <p className="text-[9px] text-sage-500 font-medium">Current</p>
        </div>
        <div className="flex-1 bg-peach-50 rounded-2xl py-2.5">
          <p className="text-sm font-extrabold text-peach-600 tabular-nums">10,000</p>
        ...
        <AreaChart data={chartData}>
          <defs>
            <linearGradient id="stepGradient" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#6f8d5e" stopOpacity={0.2} />
              <stop offset="100%" stopColor="#6f8d5e" stopOpacity={0} />
            </linearGradient>
          </defs>
          <XAxis dataKey="week" hide />
          <ReferenceLine y={7500} stroke="#d4a843" strokeDasharray="3 3" strokeWidth={1} />
          <Area type="monotone" dataKey="steps" stroke="#6f8d5e" fill="url(#stepGradient)" strokeWidth={2} dot={false} />
