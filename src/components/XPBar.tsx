...
export default function QPBar({ current, max, level }: QPBarProps) {
  ...
  return (
    <div className="flex items-center gap-3.5">
      <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-royal-400 to-royal-600 text-white flex items-center justify-center text-sm font-extrabold shrink-0 shadow-md">
        {level}
      </div>
      <div className="flex-1">
        ...
        <div className="h-2.5 bg-sand-200 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-royal-400 to-royal-500 rounded-full transition-all duration-700"
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
