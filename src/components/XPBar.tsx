...
export default function QPBar({ lifetimeMiles }: QPBarProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-9 h-9 rounded-xl bg-forest-500 text-white flex items-center justify-center text-xs font-bold shrink-0 leading-tight text-center px-0.5">
      ...
      <div className="flex-1">
        ...
        {next && (
          <p className="text-xs text-warm-400 font-medium mt-1 leading-normal">{milesToNext.toLocaleString()} mi to {next.title}</p>
        )}
      </div>
    </div>
  )
